...
function fbcommentbox($content) {
	...
	if ($post_meta['_disable_fbc'] !='on') {
		...
		$content .= "<!-- Facebook Comments Plugin for WordPress: # -->".$commenttitle.$commentcount;
	    ...
	    if ($options['linklove'] != 'no') {
	        if ($options['linklove'] != 'off') {
	            if (empty($fbcomments[linklove])) {
	      			$content .= '<p>Powered by <a href="https://wordpress.org/plugins/new-facebook-comments/">New Facebook Comments</a></p>';
    			}
    		}
    	}
  	}
...
function fbcommentshortcode($fbatts) {
	...
	if (!isset($fbcomments['count'])) {$fbcomments['count'] = "";}
		...
		if (!isset($commentcount)) {$commentcount = "";}
		$fbcommentbox = "<!-- Facebook Comments Plugin for WordPress: # -->".$commenttitle.$commentcount;
	...
	if (!empty($fbcomments['linklove'])) {
      $fbcommentbox .= '<p>Powered by <a href="https://wordpress.org/plugins/new-facebook-comments/">New Facebook Comments</a></p>';
	}
  return $fbcommentbox;
}
